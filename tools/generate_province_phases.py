...
def add_strait_adjacencies(
    path: Path, adjacency: dict[int, set[int]]
) -> set[tuple[int, int]]:
    ...
    with path.open(encoding="utf-8-sig", newline="") as source:
        ...
        if not reader.fieldnames or not required.issubset(reader.fieldnames):
            raise ValueError(f"Invalid adjacency table header in {path}")
        for line_number, row in enumerate(reader, start=2):
            if row["Type"].strip().lower() != "sea":
                continue
            try:
                first = int(row["From"].strip().removeprefix("x"), 16)
                second = int(row["To"].strip().removeprefix("x"), 16)
            except ValueError as error:
                raise ValueError(
                    f"Invalid sea adjacency at {path}:{line_number}"
                ) from error
            if first == second:
                raise ValueError(
                    f"Self-adjacent province x{first:06X} at {path}:{line_number}"
                )
            if first in adjacency and second in adjacency:
...
def bound_owner_neighbours(neighbours: tuple[int, ...]) -> str:
    tests = " ".join(
        f"var:ffcs_random_sponsor_v5 ?= {p(neighbour)}.state.owner"
        for neighbour in neighbours
    )
    return f"OR = {{ {tests} }}"
    ...
    for region in regions:
        ...
        lines.extend(
            [
                ...
                "\t\tremove_variable = ffcs_random_candidate_v5",
                "\t\tset_variable = { name = ffcs_random_sponsor_v5 value = $COUNTRY$ }",
                "\t\tset_variable = { name = ffcs_random_original_owner_v5 value = $TARGET$ }",
                "\t\tset_variable = { name = ffcs_random_project_v5 value = $PROJECT$ }",
                "\t\trandom_list = {",
            ]
        )
        for ordinal, (candidate, neighbours) in enumerate(entries, 1):
            lines.append(
                f"\t\t\t1 = {{ trigger = {{ "
                "var:ffcs_random_project_v5 = { var:ffcs_transfer_budget_v2 > 0 } "
                "num_provinces > 1 "
                f"var:ffcs_random_original_owner_v5 ?= {p(candidate)}.state.owner "
                f"{neighbour_test(neighbours)} }} "
                "set_variable = { name = ffcs_random_candidate_v5 "
                f"value = {ordinal} }} }}"
            )
        lines.append("\t\t}")
        lines.extend(
            [
                "\t\tremove_variable = ffcs_random_sponsor_v5",
                "\t\tremove_variable = ffcs_random_original_owner_v5",
                "\t\tremove_variable = ffcs_random_project_v5",
            ]
        )
        for ordinal, (candidate, _) in enumerate(entries, 1):
            inner_keyword = "if" if ordinal == 1 else "else_if"
            lines.append(
                f"\t\t{inner_keyword} = {{ limit = {{ "
                "has_variable = ffcs_random_candidate_v5 "
                f"var:ffcs_random_candidate_v5 = {ordinal} }} "
                "remove_variable = ffcs_random_candidate_v5 "
                f"{transfer(candidate)} }}"
            )
        lines.append("\t}")
        outer_branch += 1
    lines.extend(["}", ""])
...
def render_outputs(
    ...
    trigger_lines = [
        ...
        "# Literal pixel-border and unconditional-strait topology for route and frontier checks.",
        "",
    ]
    ...
    effect_lines = [
        "# AUTO-GENERATED by tools/generate_province_phases.py; do not edit.",
        "# Random selection records a candidate; ownership transfer runs afterward.",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--adjacencies", required=True, type=Path)
    parser.add_argument(
        "--effects-output",
        type=Path,
        default=root
    ...
    strait_edges = add_strait_adjacencies(args.adjacencies, adjacency)
    validate_adjacency(adjacency)
    ...
    manifest = {
        "generator_schema": 6,
        "selection_mode": "root_bound_staged_uniform_random_frontier",
        "state_region_count": len(regions),
        "province_count": len(resolved),
        "adjacencies_input_sha256": hashlib.sha256(
            args.adjacencies.read_bytes()
        ).hexdigest(),
        "strait_adjacency_edge_count": len(strait_edges),
    ...
    print(
        ...
        f"{manifest['adjacency_edge_count']} edges including {len(strait_edges)} straits"
    )
